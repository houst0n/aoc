-module(three).
-export([solve/0]).
-define(INPUT, "116	1470	2610	179	2161	2690	831	1824	2361	1050	2201	118	145	2275	2625	2333
...
solve() ->
	Differences = iter_lines(string:tokens(?INPUT, "\n"), 0),
	part1(Differences, 0).
...
iter_lines([H|T], Acc) ->
	Entries = list_to_ints(string:tokens(H, "\t"), []),
	Res = iter_entries(Entries, {low, high}),
	iter_lines(T, [Res|Acc]);
iter_lines([], Res) ->
	Res.
	
iter_entries([H|T], {low, high}) -> 
	iter_entries(T, {H, H});
iter_entries([H|T], {Lo, Hi}) when H =< Lo -> 
	iter_entries(T, {H, Hi});
iter_entries([H|T], {Lo, Hi}) when H >= Hi -> 
	iter_entries(T, {Lo, H});
iter_entries([_|T], {Lo, Hi}) -> 
	iter_entries(T, {Lo, Hi});
iter_entries([], Res) ->
	Res.

list_to_ints([], Acc) -> Acc;
list_to_ints([H|T], Acc) ->
  {N, _} = string:to_integer([H]),
  list_to_ints(T, [N|Acc]).
